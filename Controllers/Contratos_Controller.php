<!--17-01-2019/Bravo/Controlador donde se gestiona las acciones de los contratos -->
    ...
    switch ($_REQUEST['action']) {
        ...
        case 'SEARCH':
            ...
            if (!$_POST) {    //Si se envia por GET se llama a la vista EDIT para que se envie por POST
                ...
                $contratos = new Contratos_Model('', $_REQUEST['centro'], $_REQUEST['tipo'], $_REQUEST['estado'], $_REQUEST['cifEmpresa'],'', $_REQUEST['periodoinicio'], $_REQUEST['periodofin'],$_REQUEST['frecuencia'], $_REQUEST['importe']);
        ...
        case 'EDIT':
            ...
            if (!isset($_REQUEST['codcontrato'])) { //Si no esta definido el email (o alguien modifica el enlace) vuelve al index.php
                new MESSAGE('No existe el contrato', '../index.php');
            } else {
                $contratos = new Contratos_Model($_REQUEST['codcontrato'], '', '', '', '', '', '', '', '','');    //creamos un objeto del modelo con el email
                ...
                if ($contratos == 'No existe el contrato') {  //Si no se encuentra la tupla
                    ...
                    if (!$_POST) { //Si se envia por GET se llama a la vista ADD para que se envie por POST
                        $contratos = new Contratos_Model($_REQUEST['codcontrato'], '', '', '', '', '', '', '', '',''); //creamos un objeto del modelo con el codigo de contrato
                        ...
                        if (!isset($_FILES['documento']['name']) || $_FILES['documento']['name'] == '') { //Si el resguardo que viene del formulario EDIT viene sin definir o vacío, nso quedamos con el resguardo que ya teniamos
                            $contratos = new Contratos_Model($_REQUEST['codcontrato'], $_REQUEST['centro'], $_REQUEST['tipo'], $_REQUEST['estado'], $_REQUEST['cifEmpresa'], $documento, $_REQUEST['periodoinicio'], $_REQUEST['periodofin'],$_REQUEST['frecuencia'], $_REQUEST['importe'],$_REQUEST['frecuencia']);
                        } else {
